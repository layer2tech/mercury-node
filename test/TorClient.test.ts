import TorClient, { TOR_ENDPOINT }  from "../src/LDK/bitcoin_clients/TorClient.mjs";
...
describe("Tor Client", () => {
    ...
    it("getBlockHeight", async () => {
        blockHeight = await torClient.getBlockHeight();
        expect(blockHeight).toEqual(expect.any(Number));
    });

    it("getLatestBlockHeader", async () => {
        latestBlockHeader = await torClient.getLatestBlockHeader(blockHeight);
        expect(latestBlockHeader).toEqual(expect.any(String));
    });

    it("getTxIdData", async () => {
        txid = (await TorClient.get(`${TOR_ENDPOINT}/electrs/block/${bestBlockHash}/txids`)).data[0];
