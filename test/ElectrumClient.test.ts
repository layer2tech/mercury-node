...
describe("Electrum Client", () => {
    ...
    it("getBlockHeight", async () => {
        blockHeight = await electrumClient.getBlockHeight();
        expect(blockHeight).toEqual(expect.any(Number));
    });

    it("getLatestBlockHeader", async () => {
        latestBlockHeader = await electrumClient.getLatestBlockHeader(blockHeight);
        expect(latestBlockHeader).toEqual(expect.any(String));
    });

    it("getTxIdData", async () => {
        txid = (await ElectrumClient.get(`rest/block/${bestBlockHash}.json`)).data.tx[0].txid;
